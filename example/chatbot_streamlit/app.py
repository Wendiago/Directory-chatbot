...
if "llm_provider" not in st.session_state:
    st.session_state.llm_provider = "ollama"  # Changed from "openai" to "ollama"
...
def render_sidebar(mcp_tools: Optional[Dict[str, List[MCPTool]]] = None):
    ...
    with st.sidebar:
        ...
        with llm_tab:
            ...
            config = st.session_state.chatbot_config
            # Only show Ollama configuration
...
def get_config_hash(config: Configuration, provider: str = "ollama") -> int:
    """Generate a hash based on Ollama configuration settings."""
    relevant_config = {
        "provider": provider,
...
async def process_chat(user_input: str):
    ...
    final_status_message = "Failed before status was set"
    ...
    try:
        ...
        if "status_placeholder" in locals():
            try:
                ...
                logging.warning(f"Failed to update status_placeholder: {update_error}")
